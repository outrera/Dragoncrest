...
func load_scene(path, with_transition=true):
	emit_signal("START",path)
	emit_signal("TRANSITION", 1)
	if with_transition:
		yield(get_node("/root/manager/gui/transition"), "TRANSITION_DONE")
	progress = 0
	done = false
...
func on_transition_done():
	print("Transition finished methinks")
